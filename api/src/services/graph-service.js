...
module.exports = class GraphService {
  ...
  /**
   * Obtiene las viviendas más relacionadas con una vivienda dada
   * basándose en especificaciones compartidas (mínimo 2 en común)
   * Ordenadas por cantidad de especificaciones compartidas
   */
  async getRelatedElements (elementId, minSharedSpecs = 2, limit = 50) {
    try {
      const query = `
        MATCH (p:Element {id: $elementId})-[:HAS_SPECIFICATION]->(s:Specification)
        MATCH (p2:Element)-[:HAS_SPECIFICATION]->(s)
        WHERE p.id <> p2.id
        WITH p2, count(DISTINCT s) as sharedSpecs
        WHERE sharedSpecs >= $minSharedSpecs
        RETURN p2.id as propertyId, sharedSpecs
        ORDER BY sharedSpecs DESC
        LIMIT toInteger($limit)
      `

      const records = await this.session.run(query, {
        elementId: String(elementId),
        minSharedSpecs: neo4j.int(minSharedSpecs),
      ...
      console.error('Error en getRelatedElements:', err)
      throw err
    }
  }
